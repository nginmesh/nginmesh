...
class Bookinfo:
    def deploy_bookinfo(self):
         subprocess.call("kubectl apply -f ../bookinfo.yaml > /dev/null 2>&1 | exit 0",universal_newlines=True,shell=True)
...
class Rule:
     def add(self,rule_name):
         subprocess.call("kubectl create -f ../"+rule_name+" > /dev/null 2>&1 | exit 0",universal_newlines=True,shell=True)
     def delete(self,rule_name):
         subprocess.call("kubectl delete -f ../"+rule_name+" > /dev/null 2>&1 | exit 0",universal_newlines=True,shell=True)
